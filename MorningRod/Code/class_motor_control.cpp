...
void MotorControl::setup_pins(int chipCS, int CLOCKOUT, int ENABLE_PIN)
{
    _chipCS = chipCS;
    _CLOCKOUT = CLOCKOUT;
...
{
    _requested_position = position long new_travel = (position - _absolute_position) / _travel;
    auto m = Movement(_current_open, _velocity, -new_travel, _stallguard);
    m.move();
    _motor_running = true;
}
void MotorControl::move_open()
{
    ...
    auto m = Movement(_current_open, _velocity, -_travel, _stallguard);
    digitalWrite(_ENABLE_PIN, LOW); // enable the TMC5130
    m.move();
    _full_close = false;
    _full_open = true;
    _motor_running = true;
...
void MotorControl::move_close()
{
    ...
    auto m = Movement(_current_close, _velocity, _travel, _stallguard);
    digitalWrite(_ENABLE_PIN, LOW); // enable the TMC5130
    m.move();
    _full_close = true;
    _full_open = false;
...
void MotorControl::move_stop()
{
    ...
    digitalWrite(_ENABLE_PIN, HIGH);  // disable the motor driver if possible
    _motor_running = false;
}
...
Movement::Movement(unsigned long current, long velocity, long travel, long stallguard)
{
    _current = current;
    _velocity = velocity;
    _travel = travel;
